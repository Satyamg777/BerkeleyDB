...
inline void LargeMatrix::write_matrix(char matname,int noof_rows,int noof_columns,int noof_block_rows,int noof_block_columns)//Method used to store matrix block by block in to database
{
	...
	for(i=0;i<noof_block_rows;i++)
	delete[] bloc.blockdata[i];
	delete[] bloc.blockdata;
}
...
int main(int argc, char **argv)
{
  	...
  	if( argc != 10 )
  	{
    		...
    		cout<<"[MATRIX-A-BLOCKS-PER-ROW] [MATRIX-A-BLOCKS-PER-COLUMN] [MATRIX-B-BLOCKS-PER-ROW] [MATRIX-B-BLOCKS-PER-COLUMN] ";
    		cout<<"[DBPATH] \n";
    		exit(-1); 
  	} 
  	...
  	size_t  noof_a_column_blocks=atoi( argv[6] );
  
  	size_t  noof_b_row_blocks=atoi( argv[7] );
  	size_t  noof_b_column_blocks=atoi( argv[8] );
  
  	string  dbpath = argv[9];
	...
	cout<<"\n storing matrices to database completed \n";   	
	...
	cout<<"\n wait matrices are reading from database \n";
	...
	cout<<"\n reading matrices from database completed \n";   	
	...
	cout<<"\n wait matrices are getting multiplied \n";
        //Matrix multiplication block by block
      	...
      	{   
         	...
         	for(global_j=0; global_j<noof_b_column_blocks; global_j++)
         	{
           		...
           		if(a_block_no % noof_a_column_blocks==0)
           		change_c_blk_no=a_block_no/noof_a_column_blocks;
           		if(b_block_no % noof_b_column_blocks==0)
           		c_block_no=change_c_blk_no*noof_b_column_blocks;   
         	}
         	if(b_block_no==(noof_b_row_blocks*noof_b_column_blocks)) 
         	b_block_no=0;
       	}
	cout<<"\n matrices multiplication completed \n";
